...
def model(i):
  t1_test = np.linspace(np.min(t1), max(t1), m)
  t2_test = np.linspace(np.min(t2), max(t2), m)
  A=matrice(t1,t2,i)
  A_test = matrice(t1_test,t2_test,i)
  A = A[:, 1:] # pentru LinearRegression()
  A_test = A_test[:, 1:] # pentru LinearRegression()
  ...
  coefs = np.hstack((lin.intercept_, lin.coef_.flatten()))

  y_pred = lin.predict(X_train) # pentru datele de training; trebuie sa coincida cu X_train * coefs
  y_pred2 = lin.predict(X_test) # pentru datele de test; trebuie sa coincida cu X_test * coefs

  x = np.linalg.solve(np.dot(A.T,A),np.dot(A.T,b))
  data=error(y_pred,y_pred2,i)
